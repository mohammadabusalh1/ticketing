...
it("returns a 201 on successful ticket creation", async () => {
  ...
  // This will automatically log error details if status doesn't match
  global.expectWithErrorLog(response, 201);
...
it("publishes an event", async () => {
  ...
  // Use dynamic import for ESM compatibility with jest.unstable_mockModule
  const { NatsWrapper } = await import("../../nats-wrapper.ts");
